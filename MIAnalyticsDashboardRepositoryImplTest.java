...
public class MIAnalyticsDashboardRepositoryImplTest {
    ...
    public void testUpsertMiSnapshot_Success() throws Exception {
        ...
        when(dfClient.upsert()).thenReturn(mockBuilder);
    ...
    public void testFetchStatistics_Success() throws Exception {
        ...
        MIAnalyticsDashboardRepositoryImpl spyRepo = spy(repo);
        doReturn(mockScanRequestBuilder).when(spyRepo).getScanRequestBuilder(eq(metadata), eq("testDatabase"), eq("testCollection"));
    ...
    public void testProcessFoMiData_EMIR_ETD() throws Exception {
        ...
        when(itrClient.getData(any())).thenReturn(mockInputStream);

        MIAnalyticsDashboardRepositoryImpl spyRepo = spy(repo);
        List<FrontOfficeMIDashboardAnalytics> capturedAnalytics = new ArrayList<>();
        doAnswer(invocation -> {
            FrontOfficeMIDashboardAnalytics arg = invocation.getArgument(0);
            capturedAnalytics.add(arg);
            return mock(RecordId.class);
        }).when(spyRepo).upsertFoMiSnapshot(any());

        spyRepo.processFoMiData(metadata, TEST_DATE, assetClass);

        assertEquals(4, capturedAnalytics.size());
        FrontOfficeMIDashboardAnalytics plcReportable = capturedAnalytics.stream()
        ...
        assertEquals(Optional.of(1), plcReportable.getMiAnalyticsState().getReportableCount());
        assertEquals(Optional.of(0), plcReportable.getMiAnalyticsState().getNonReportableCount());

        FrontOfficeMIDashboardAnalytics nvNonReportable = capturedAnalytics.stream()
        ...
        assertEquals(Optional.of(0), nvNonReportable.getMiAnalyticsState().getReportableCount());
        assertEquals(Optional.of(1), nvNonReportable.getMiAnalyticsState().getNonReportableCount());
    }
    ...
    public void testProcessFoMiData_EMIR_FX() throws Exception {
        ...
        assertEquals(4, capturedAnalytics.size());
        FrontOfficeMIDashboardAnalytics plcNonReportable = capturedAnalytics.stream()
                .filter(a -> a.getSubjectIdentifier().getEntity().equals(NWM_PLC) && a.getSubjectIdentifier().getRegulation().equals(EMIR))
                .findFirst().orElse(null);
        assertNotNull(plcNonReportable);
        assertEquals(Optional.of(0), plcNonReportable.getMiAnalyticsState().getReportableCount());
        assertEquals(Optional.of(1), plcNonReportable.getMiAnalyticsState().getNonReportableCount());
    }
    ...
    public void testProcessFoMiData_CFTC_CSA_FX() throws Exception {
        ...
        assertEquals(6, capturedAnalytics.size());
        FrontOfficeMIDashboardAnalytics plcCftc = capturedAnalytics.stream()
                .filter(a -> a.getSubjectIdentifier().getRegulation().equals(DF))
                .findFirst().orElse(null);
        assertNotNull(plcCftc);
        assertEquals(Optional.of(1), plcCftc.getMiAnalyticsState().getReportableCount());
    }
    ...
    public void testProcessFoMiData_InvalidJurisdiction() throws Exception {
        ...
        ReflectionTestUtils.setField(MIConstants.class, "FO_ASSETWISE_MAP", foAssetWiseMap);
    ...
    public void testDeserialize() throws Exception {
        ...
        MIDashboardAnalytics deserialized = repo.deserialize(document);
    ...
    public void testDeserialize_InvalidJson() throws Exception {
        ...
        repo.deserialize(invalidDocument);
    }
